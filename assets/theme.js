		...
		if (this.getAttribute('country') !== '') {
			this.countryElement.selectedIndex = Math.max(
				...
				Array.from(this.countryElement.options).findIndex(
					option =>
						option.textContent === this.getAttribute('country'),
				),
			)
...
function cachedFetch(url, options) {
	...
	if (cachedMap.has(cacheKey)) {
		return Promise.resolve(
			new Response(new Blob([cachedMap.get(cacheKey)])),
		)
	}
...
import { timeline } from '//cdn.shopify.com/s/files/1/0765/4427/8817/t/4/assets/vendor.min.js?v=101728284033374364961688715548'
...
import {
	...
	stagger,
} from '//cdn.shopify.com/s/files/1/0765/4427/8817/t/4/assets/vendor.min.js?v=101728284033374364961688715548'
...
import { stagger as stagger2 } from '//cdn.shopify.com/s/files/1/0765/4427/8817/t/4/assets/vendor.min.js?v=101728284033374364961688715548'
...
var CustomCursor = class extends HTMLElement {
	...
	connectedCallback() {
		...
		this.parentElement.addEventListener(
			'pointerleave',
			__privateMethod(this, _onPointerLeave, onPointerLeave_fn).bind(
				this,
			),
			{ signal: __privateGet(this, _abortController).signal },
		)
...
import { inView as inView2 } from '//cdn.shopify.com/s/files/1/0765/4427/8817/t/4/assets/vendor.min.js?v=101728284033374364961688715548'
var _resizeObserver,
...
var SafeSticky = class extends HTMLElement {
	constructor() {
		...
		__privateAdd(
			...
			_checkPositionListener,
			throttle(
				__privateMethod(this, _checkPosition, checkPosition_fn).bind(
					this,
				),
			),
		)
...
import { scroll } from '//cdn.shopify.com/s/files/1/0765/4427/8817/t/4/assets/vendor.min.js?v=101728284033374364961688715548'
...
var ScrollCarousel = class extends BaseCarousel {
	constructor() {
		super()
		if (window.ResizeObserver) {
			new ResizeObserver(throttle(this._adjustHeight.bind(this))).observe(
				this,
			)
		}
	}
...
import { animate as animate3 } from '//cdn.shopify.com/s/files/1/0765/4427/8817/t/4/assets/vendor.min.js?v=101728284033374364961688715548'
...
import { animate as animate4 } from '//cdn.shopify.com/s/files/1/0765/4427/8817/t/4/assets/vendor.min.js?v=101728284033374364961688715548'
	...
	disconnectedCallback() {
		...
		if (__privateGet(this, _isLocked)) {
			...
			document.documentElement.classList.toggle(
				'lock',
				--__privateWrapper(_DialogElement, _lockLayerCount)._ > 0,
			)
		}
	}
	show(animate11 = true) {
				...
				if (
					...
					if (this.shouldLock) {
						__privateSet(
							_DialogElement,
							_lockLayerCount,
							__privateGet(_DialogElement, _lockLayerCount) + 1,
						)
					...
					if (this.shouldLock) {
						...
						document.documentElement.classList.toggle(
							'lock',
							--__privateWrapper(_DialogElement, _lockLayerCount)
								._ > 0,
						)
					}
				}
...
import { Delegate as Delegate2 } from '//cdn.shopify.com/s/files/1/0765/4427/8817/t/4/assets/vendor.min.js?v=101728284033374364961688715548'
...
var CartDrawer = class extends Drawer {
	...
	async _onCartChanged(event) {
		...
		if (event.detail.cart['item_count'] > 0) {
			const currentLineItemsElement = this.querySelector(
					'.cart-drawer__line-items',
				),
				updatedLineItemsElement = updatedDrawerContent.querySelector(
					'.cart-drawer__line-items',
				)
			if (!currentLineItemsElement) {
				...
				setTimeout(
					() => {
						currentLineItemsElement.innerHTML =
							updatedLineItemsElement.innerHTML
					},
					event.detail.baseEvent === 'variant:add' ? 0 : 1250,
				)
				this.querySelector('[slot="footer"]').replaceChildren(
					...updatedDrawerContent.querySelector('[slot="footer"]')
						.childNodes,
				)
			}
		} else {
	...
	async _onCartRefresh() {
		...
		tempDiv.innerHTML = await (
			await fetch(
				`${window.Shopify.routes.root}?section_id=${extractSectionId(
					this,
				)}`,
			)
		).text()
...
import { animate as animate5 } from '//cdn.shopify.com/s/files/1/0765/4427/8817/t/4/assets/vendor.min.js?v=101728284033374364961688715548'
...
var QuantityInput = class extends HTMLInputElement {
	...
	set quantity(quantity) {
		...
		if (window && window.updateCounter) {
	...
	connectedCallback() {
		if (!__privateGet(this, _accessibilityInitialized)) {
			...
			__privateGet(this, _hiddenInput)?.addEventListener(
				'change',
				__privateMethod(this, _onInputChanged, onInputChanged_fn).bind(
					this,
				),
			)
...
import { Delegate as Delegate3 } from '//cdn.shopify.com/s/files/1/0765/4427/8817/t/4/assets/vendor.min.js?v=101728284033374364961688715548'
...
var SoldOutBadge = class extends HTMLElement {
	constructor() {
		...
		__publicField(
			...
			__privateMethod(this, _onVariantChanged, onVariantChanged_fn).bind(
				this,
			),
		)
	}
...
import { PhotoSwipeLightbox } from '//cdn.shopify.com/s/files/1/0765/4427/8817/t/4/assets/vendor.min.js?v=101728284033374364961688715548'
var ProductGallery = class extends HTMLElement {
	...
	get photoswipe() {
		...
		const photoswipe = new PhotoSwipeLightbox({
			pswpModule: () =>
				import(
					'//cdn.shopify.com/s/files/1/0765/4427/8817/t/4/assets/photoswipe.min.js?v=96595610301167188461688715546'
				),
			bgOpacity: 1,
        ...
        `,
				onInit: (el, pswp) => {
					el.querySelector('.pagination__page-count').innerText =
						pswp.getNumItems()
					el.querySelector('[rel="prev"]')?.addEventListener(
						'click',
						() => pswp.prev(),
					)
					el.querySelector('[rel="next"]')?.addEventListener(
						'click',
						() => pswp.next(),
					)
					pswp.on('change', () => {
		...
		if (event.scale > 1.5) {
			this.dispatchEvent(
				new CustomEvent('lightbox:open', {
					bubbles: true,
					detail: { index: this.selectedIndexAmongVisible },
				}),
			)
...
var ProductZoomButton = class extends HTMLButtonElement {
	...
	_onButtonClicked() {
		let media = this.closest('.product-gallery__media'),
			openIndex
		if (media) {
			const visibleImages = this.closest(
				'media-carousel',
			).visibleItems.filter(
...
var ProductQuickAdd = class extends HTMLElement {
	constructor() {
		...
		__privateAdd(
			...
			new IntersectionObserver(this._onObserved.bind(this)),
		)
	}
	connectedCallback() {
	...
	_onObserved(entries) {
		...
		this.classList.toggle(
			'is-visible',
			__privateGet(this, _scopeFromPassed) &&
				!__privateGet(this, _scopeToReached),
		)
	}
}
...
var VariantPicker = class extends HTMLElement {
	...
	_updateDisableSelectors() {
		...
		if (this.optionSelectors && this.optionSelectors[0]) {
			this.product['options'][0]['values'].forEach(
				(value, valueIndex) => {
					const hasAtLeastOneCombination = this.product[
							'variants'
						].some(
					...
					if (this.optionSelectors[1]) {
						this.product['options'][1]['values'].forEach(
							(value2, valueIndex2) => {
								const hasAtLeastOneCombination2 = this.product[
										'variants'
									].some(
										variant =>
									...
									hasAvailableVariant2 = this.product[
										'variants'
									].some(
										variant =>
											variant['option2'] === value2 &&
								...
								if (this.optionSelectors[2]) {
									this.product['options'][2][
										'values'
									].forEach((value3, valueIndex3) => {
										const hasAtLeastOneCombination3 =
											...
											hasAvailableVariant3 = this.product[
												'variants'
											].some(
												variant =>
													variant['option3'] ===
...
import { inView as inView4 } from '//cdn.shopify.com/s/files/1/0765/4427/8817/t/4/assets/vendor.min.js?v=101728284033374364961688715548'
	...
	attributeChangedCallback(name, oldValue, newValue) {
		if (name === 'playing') {
			if (oldValue === null && newValue === '') {
				this.dispatchEvent(
					new CustomEvent('media:play', { bubbles: true }),
				)
				...
				this.dispatchEvent(
					new CustomEvent('media:pause', { bubbles: true }),
				)
			}
		}
	}
...
import { timeline as timeline6 } from '//cdn.shopify.com/s/files/1/0765/4427/8817/t/4/assets/vendor.min.js?v=101728284033374364961688715548'
...
import { animate as animate8 } from '//cdn.shopify.com/s/files/1/0765/4427/8817/t/4/assets/vendor.min.js?v=101728284033374364961688715548'
...
import { animate as animate9 } from '//cdn.shopify.com/s/files/1/0765/4427/8817/t/4/assets/vendor.min.js?v=101728284033374364961688715548'
...
class CountdownTimer extends HTMLElement {
    ...
    <div class="timer-box" id="countdown">
      ...
      <ul>
        <li><span id="timer-days">00</span>days</li>
        <li><span id="timer-hours">00</span>Hours</li>
        <li><span id="timer-minutes">00</span>Minutes</li>
        <li><span id="timer-seconds">00</span>Seconds</li>
      </ul>
    </div>
  `
	...
	updateCountdownTimer() {
		...
		let countDownDate = new Date(this.dataset.endTime)

		let now = new Date().getTime()
		let timeleft = countDownDate - now
		if (timeleft < 0) {
			clearInterval(this.timer)
			this.closest('.countdown_timer_sec').querySelector('h4').innerHTML =
				'Raffle ended!'
			return
		}

		const timerDisplayDays = this.shadowRoot.getElementById('timer-days')
		const timerDisplaySeconds =
			this.shadowRoot.getElementById('timer-seconds')
		const timerDisplayMinutes =
			this.shadowRoot.getElementById('timer-minutes')
		...
		timerDisplayDays.innerHTML = days.toLocaleString('en-US', {
			minimumIntegerDigits: 2,
			useGrouping: false,
		})
		timerDisplaySeconds.innerHTML = seconds.toLocaleString('en-US', {
			minimumIntegerDigits: 2,
			useGrouping: false,
		})
		timerDisplayMinutes.innerHTML = minutes.toLocaleString('en-US', {
			minimumIntegerDigits: 2,
			useGrouping: false,
		})
		timerDisplayHour.innerHTML = hours.toLocaleString('en-US', {
			minimumIntegerDigits: 2,
			useGrouping: false,
		})
	}
...
// js/theme.js
import { Delegate as Delegate4 } from '//cdn.shopify.com/s/files/1/0765/4427/8817/t/4/assets/vendor.min.js?v=101728284033374364961688715548'
;(() => {
